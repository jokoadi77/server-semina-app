...
const activateParticipant = async (req) => {
  ...
  if (!check) throw new NotFoundErrors('Partisipan belum terdaftar');

  if (check && check.otp !== otp) throw new BadRequestErrors('Kode otp salah');
...
const checkoutOrder = async (req) => {
  ...
  if (!checkingEvent) {
    throw new NotFoundErrors('Tidak ada acara dengan id : ' + event);
  }
    ...
    checkingEvent.tickets.forEach((ticket) => {
      if (tic.ticketCategories.type === ticket.type) {
        if (tic.sumTicket > ticket.stock) {
          throw new NotFoundErrors('Stock event tidak mencukupi');
        } else {
